...
export function TransferToConditioningModal({ batchId, batchNumber, recipeName, isOpen, onClose, onComplete }: Props) {
  // ✅ ALL HOOKS FIRST
  const [finalGravity, setFinalGravity] = useState('')
  const [temperature, setTemperature] = useState('2')
  const [selectedTankId, setSelectedTankId] = useState('')
  const [stayInSameTank, setStayInSameTank] = useState(true)
  ...
  const totalVolume = useMemo(() => {
    return parseFloat(batch?.volume?.toString() || '100')
  }, [batch])
  ...
  // გაყოფისთვის ხელმისაწვდომი ავზები
  ...
  const fetchActiveAssignments = useCallback(async () => {
    try {
      ...
      // ✅ დაამატე phase=CONDITIONING filter
      const response = await fetch('/api/tanks/active-assignments?phase=CONDITIONING')
      if (response.ok) {
        const data = await response.json()
        // ✅ გამორიცხე მიმდინარე batch
  ...
  useEffect(() => {
    if (isOpen && batch) {
      setFinalGravity(batch.fg?.toString() || '')
      setTemperature('2')
      setNotes('')
      setStayInSameTank(true)
      setSelectedTankId('')
      setSplitMode(false)
      setBlendMode(false)
      setTankAllocations([])
  ...
  const addTankAllocation = useCallback((tankId: string) => {
    if (tankAllocations.some(a => a.tankId === tankId)) return
    
    const tank = availableTanks.find(t => t.id === tankId)
    const defaultVolume = Math.min(remainingVolume, tank?.capacity || remainingVolume)
    
    setTankAllocations(prev => [...prev, {
      tankId,
      volume: defaultVolume > 0 ? defaultVolume : totalVolume / (tankAllocations.length + 1),
    }])
  ...
  const handleConfirm = useCallback(async () => {
    ...
    } else if (!stayInSameTank && !selectedTankId) {
      alert('აირჩიეთ სამიზნე ავზი')
      return
    }
    ...
    try {
      const response = await fetch(`/api/batches/${batchId}/transfer-conditioning`, {
        ...
        body: JSON.stringify({
          // ჩვეულებრივი რეჟიმი
          targetTankId: stayInSameTank ? batch?.tankId : selectedTankId,
          stayInSameTank,
          
          // გაყოფა
          isSplit: splitMode && tankAllocations.length > 1,
          ...
          // პარამეტრები
          finalGravity: finalGravity ? parseFloat(finalGravity) : undefined,
      ...
      console.log('[TRANSFER_CONDITIONING] ✅ Success:', result)

      // ✅ გამოიძახე onComplete CALLBACK - ეს რეფრეშავს მონაცემებს
      if (onComplete) {
        await onComplete()
      }
      ...
      // ✅ დამატებით - გვერდის რეფრეშს თუ callback არ მუშაობს
      setTimeout(() => {
  ...
  }, [batchId, splitMode, blendMode, tankAllocations, remainingVolume, selectedBlendTarget, stayInSameTank, selectedTankId, batch?.tankId, finalGravity, temperature, notes, onComplete, onClose])
  ...
  return (
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
      <div className="bg-slate-800 rounded-xl p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto border border-slate-700 shadow-2xl">
        <h2 className="text-xl font-bold text-white mb-2">❄️ კონდიცირებაზე გადატანა</h2>
        <p className="text-slate-400 mb-1">{batchNumber} - {recipeName}</p>
        <p className="text-slate-500 text-sm mb-4">მოცულობა: {totalVolume}L | მიმდინარე ავზი: {batch?.tank?.name || 'N/A'}</p>

        {/* ═══════════════════════════════════════════════════════ */}
        {/* გაფართოებული პარამეტრები */}
        {/* ═══════════════════════════════════════════════════════ */}
        <div className="mb-4 p-3 bg-slate-700/50 rounded-lg space-y-2">
          <div className="text-sm text-slate-400 font-medium mb-2">გაფართოებული პარამეტრები</div>
          
          {/* გაყოფა */}
          <label className="flex items-center gap-2 p-2 bg-blue-500/10 border border-blue-500/30 rounded cursor-pointer hover:bg-blue-500/20">
            <input
              type="checkbox"
              checked={splitMode}
              onChange={(e) => {
          ...
          {/* შერევა */}
          <label className="flex items-center gap-2 p-2 bg-purple-500/10 border border-purple-500/30 rounded cursor-pointer hover:bg-purple-500/20">
            <input
              type="checkbox"
              checked={blendMode}
              onChange={(e) => {
          ...
          <div className="mb-4 p-3 bg-blue-500/10 border border-blue-500/30 rounded-lg">
                  ...
                  return (
                    <div key={allocation.tankId} className="flex items-center gap-2 p-2 bg-slate-800 rounded">
                      <div className="flex-1">
                        <div className="text-sm text-white">{tank?.name}</div>
                        <div className="text-xs text-slate-500">მაქს. {tank?.capacity}L</div>
                      </div>
                      <input
                        type="number"
            ...
            {availableTanksForSplit.length > 0 && (
              <select
                value=""
                onChange={(e) => {
                  if (e.target.value) addTankAllocation(e.target.value)
                }}
          ...
          <div className="mb-4 p-3 bg-purple-500/10 border border-purple-500/30 rounded-lg">
            ...
            {loadingAssignments ? (
              ...
              <p className="text-sm text-slate-400 p-3 bg-slate-700/50 rounded">
                არ არის აქტიური ბაჩები კონდიცირებაში შერევისთვის
              </p>
            )}
          </div>
        )}
          ...
          <div className="mb-4">
            <label className="flex items-center gap-2 mb-3 cursor-pointer">
              <input
                ...
                onChange={(e) => setStayInSameTank(e.target.checked)}
                className="w-4 h-4 rounded"
              />
              <span className="text-slate-300">იმავე ავზში დარჩენა ({batch?.tank?.name || 'N/A'})</span>
            </label>
            
            {!stayInSameTank && (
              <div>
                <label className="block text-sm text-slate-300 mb-2">სამიზნე ავზი *</label>
                {loadingEquipment ? (
                  ...
                  >
                    ...
                    {availableTanks.filter(t => t.id !== batch?.tankId).map(tank => (
                      <option key={tank.id} value={tank.id}>
        ...
        <div className="grid grid-cols-2 gap-4 mb-4">
          <div>
            <label className="block text-sm text-slate-300 mb-2">საბოლოო სიმკვრივე (FG)</label>
            <input
              type="text"
              value={finalGravity}
          ...
          <div>
            <label className="block text-sm text-slate-300 mb-2">ტემპერატურა (°C)</label>
            <input
              type="number"
              value={temperature}
        ...
        <div className="flex gap-3 justify-end">
          ...
          <Button 
            ...
            disabled={
              ...
              (!splitMode && !blendMode && !stayInSameTank && !selectedTankId)
            }
            className="bg-blue-600 hover:bg-blue-700"
          >
            {isSubmitting ? 'იტვირთება...' : 
              splitMode ? '🔀 გაყოფა და გადატანა' : 
              blendMode ? '🔄 შერევა' : 
              '❄️ გადატანა'
            }
          </Button>
