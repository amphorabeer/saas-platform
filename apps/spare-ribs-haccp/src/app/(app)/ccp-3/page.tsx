...
export default function CCP3Page() {
  ...
  const fetchLogs = async () => {
    setLoading(true)
    try {
      const res = await fetch('/api/haccp/monitoring?ccp=CCP-3&limit=50')
    ...
    try {
      await fetch('/api/haccp/monitoring', {
        ...
        body: JSON.stringify({
          id: editLog.id,
          ccpNumber: 'CCP-3',
          fridgeTempAm: +editLog.fridgeTempAm,
          fridgeTempPm: +editLog.fridgeTempPm,
          freezerTempAm: +editLog.freezerTempAm,
          freezerTempPm: +editLog.freezerTempPm,
          notes: editLog.notes,
        }),
      })
      await fetchLogs()
  ...
  const TempCell = ({ val, min, max, freezer }: { val: number, min?: number, max?: number, freezer?: boolean }) => {
    let ok = val != null
    if (ok && freezer) ok = val <= -18
    else if (ok) ok = (min === undefined || val >= min) && (max === undefined || val <= max)
    return <span className={ok ? 'text-green-700 font-medium' : 'text-red-600 font-medium'}>{val != null ? `${val}°C` : '—'}</span>
  }
    ...
    <div className="page">
        ...
        <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md p-5 space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="text-sm font-bold">რედ. — {editLog.batchLot}</h2>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="fg">
                <label className="fl">დილა °C</label>
                <input type="number" step="0.1" value={editLog.fridgeTempAm||''}
                  onChange={e => setEditLog({...editLog, fridgeTempAm: e.target.value})}
                  className={`inp ${+editLog.fridgeTempAm < 0 || +editLog.fridgeTempAm > 4 ? 'inp-err' : 'inp-ok'}`} />
              </div>
              <div className="fg">
                <label className="fl">საღამო °C</label>
                <input type="number" step="0.1" value={editLog.fridgeTempPm||''}
                  onChange={e => setEditLog({...editLog, fridgeTempPm: e.target.value})}
                  className={`inp ${+editLog.fridgeTempPm < 0 || +editLog.fridgeTempPm > 4 ? 'inp-err' : 'inp-ok'}`} />
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="fg">
                <label className="fl">დილა °C</label>
                <input type="number" step="0.1" value={editLog.freezerTempAm||''}
                  onChange={e => setEditLog({...editLog, freezerTempAm: e.target.value})}
                  className={`inp ${+editLog.freezerTempAm > -18 ? 'inp-err' : 'inp-ok'}`} />
              </div>
              <div className="fg">
                <label className="fl">საღამო °C</label>
                <input type="number" step="0.1" value={editLog.freezerTempPm||''}
                  onChange={e => setEditLog({...editLog, freezerTempPm: e.target.value})}
                  className={`inp ${+editLog.freezerTempPm > -18 ? 'inp-err' : 'inp-ok'}`} />
              </div>
            </div>
            <div className="fg">
      ...
      <div className="page-hdr">
        ...
        <div className="flex gap-2">
          ...
          <Link href="/ccp-3/new" className="btn-primary btn-sm"><Plus className="w-4 h-4" />ახალი ჩანაწ.</Link>
        </div>
      </div>
      ...
      <div className="card p-0">
        <table className="tbl">
          <thead>
            <tr><th>LOT №</th><th>მაც.დ.</th><th>მაც.ს.</th><th>საყ.დ.</th><th>საყ.ს.</th><th>სტ.</th><th>ოპ.</th><th>თ.</th><th>მოქმ.</th></tr>
          </thead>
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={9} className="text-center py-6 text-gray-400">
                ჩანაწ. არ — <Link href="/ccp-3/new" className="text-blue-600 hover:underline">+ ახალი</Link>
              </td></tr>
            ) : logs.map((l: any) => (
              <>
                <tr key={l.id} className="cursor-pointer hover:bg-gray-50"
                  ...
                  <td><TempCell val={l.fridgeTempAm} min={0} max={4} /></td>
                  <td><TempCell val={l.fridgeTempPm} min={0} max={4} /></td>
                  <td><TempCell val={l.freezerTempAm} freezer /></td>
                  <td><TempCell val={l.freezerTempPm} freezer /></td>
                    ...
                    <div className="flex gap-1 items-center">
                      ...
                      {!l.isCompliant && (
                        l.correctiveAction ? (
                          ...
                          <Link href={`/corrective-actions/new?ccp=CCP-3&lot=${l.batchLot}`}
                  ...
                  <tr key={`${l.id}-d`} className="bg-blue-50/40">
                    <td colSpan={9} className="px-4 py-3">
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
                        ...
                        <div><div className="text-xs text-gray-400 mb-0.5">მაც. დილა</div><TempCell val={l.fridgeTempAm} min={0} max={4} /></div>
                        <div><div className="text-xs text-gray-400 mb-0.5">მაც. საღამო</div><TempCell val={l.fridgeTempPm} min={0} max={4} /></div>
                        <div><div className="text-xs text-gray-400 mb-0.5">საყ. დილა</div><TempCell val={l.freezerTempAm} freezer /></div>
                        <div><div className="text-xs text-gray-400 mb-0.5">საყ. საღამო</div><TempCell val={l.freezerTempPm} freezer /></div>
